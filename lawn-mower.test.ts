...
class LawnMower {
    ...
    move() {
        if (this.direction === 'S') {
            return new LawnMower(0, this.y - 1, new Position(0, this.position.y - 1), "S")
        }
        return new LawnMower(0, this.y + 1, new Position(0, this.position.y + 1), "N")
    }
}
...
describe('lawn mower', () => {
    ...
    it('two mower in different spots should be different', () => {
        const lawnMower = new LawnMower(1, 0, new Position(1, 0), 'N')
    ...
    it('can move', () => {
        ...
        expect(moved).toEqual(new LawnMower(0, 1, new Position(0, 1), 'N'))
    })

    it('can move 2', () => {
        const lawnMower = new LawnMower(0, 1, new Position(0, 1), 'N')

        const moved = lawnMower.move()

        expect(moved).toEqual(new LawnMower(0, 2, new Position(0, 2), 'N'))
    })

    it('can move south', () => {
        const lawnMower = new LawnMower(0, 1, new Position(0, 1), 'S')
    ...
    it('can move south 2', () => {
        const lawnMower = new LawnMower(0, 2, new Position(0, 2), 'S')

        const moved = lawnMower.move()

        expect(moved).toEqual(new LawnMower(0, 1, new Position(0, 1), 'S'))
    })
