...
class LawnMower {
    constructor(private position: Position = new Position(0, 0), private direction: "N" | "S") {
    }
...
describe('lawn mower', () => {
    ...
    it('can move 2', () => {
        const lawnMower = new LawnMower(new Position(0, 1), 'N')

        const moved = lawnMower.move()

        expect(moved).toEqual(new LawnMower(new Position(0, 2), 'N'))
    })
    ...
    it('can move south 2', () => {
        const lawnMower = new LawnMower(new Position(0, 2), 'S')

        const moved = lawnMower.move()

        expect(moved).toEqual(new LawnMower(new Position(0, 1), 'S'))
    })
